...
private:
    ...
    ECommError Command(const char * cmd, int32 timeout, const char * reply) {
        char buf[LINE_LEN + 4];
        uint32 len = LINE_LEN;
        bzero(buf, LINE_LEN + 4);
        return Command(cmd, timeout, reply, buf, len);
    };
    ECommError Command(const char * cmd, int32 timeout) {
        return Command(cmd, timeout, "OK");
    }
    ECommError Command(const char * cmd, const char * reply) {
        return Command(cmd, RX_TIMEOUT, reply);
    }
    ECommError Command(const char * cmd) {
        return Command(cmd, "OK");
    }
    ECommError WaitString(const char * str, int32 timeout, char * buf, uint32 & len);
    ECommError WaitString(const char * str, int32 timeout) {
        char buf[LINE_LEN + 4];
        uint32 len = LINE_LEN;
        bzero(buf, LINE_LEN + 4);
        return WaitString(str, timeout, buf, len);
    };
